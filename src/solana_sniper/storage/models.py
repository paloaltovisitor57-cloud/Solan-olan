...
class ExecutionIntentRow(Base):
    """One autonomous swap (see domain.models.ExecutionIntent): written before anything is
    signed or sent, updated at every step, so a crash can be reconciled by signature."""

    __tablename__ = "execution_intents"
    intent_id: Mapped[str] = mapped_column(String(40), primary_key=True)
    session_id: Mapped[str] = mapped_column(String(64), index=True)
    signal_id: Mapped[str] = mapped_column(String(40), index=True)
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True))
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True))
    wallet_public_key: Mapped[str] = mapped_column(String(64))
    signature: Mapped[str | None] = mapped_column(String(96), nullable=True, index=True)
    last_valid_block_height: Mapped[int | None] = mapped_column(Integer, nullable=True)
    in_amount_raw: Mapped[str] = mapped_column(String(40), default="0")
